...
public class EchoWebSocket {
     ...
     * 웹소켓 세션관리를 위한 앱객체
     * - id = session
     * - 멀티스레드 환경을 위한 동기화(스레드 간 이용 순서가 정해진, 한번에 하나의 스레드만 접근 가능한) 처리된 맵이 필요하다.
     */
    ...
    public static void sendNotification(Notification noti) {
        ...
        if (session != null) {
            ...
            System.out.println(noti.getMemberId() + "는 접속중이아닙니다...");
        }
    }
    ...
    public void open(EndpointConfig config, Session session) {
        ...
        // 연결해제시 map에서 사용자제거를 위해 session의 내부맵에도 사용자 id를 등록
